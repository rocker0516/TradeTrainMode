...
def test_dual_cnn_feature_extractor():
    ...
    observation_space = gym.spaces.Dict({
        ...
        "account_state": spaces.Box(low=-np.inf, high=np.inf, shape=(16,), dtype=np.float32),
        "cost_state": spaces.Box(low=-np.inf, high=np.inf, shape=(1,), dtype=np.float32),
    })
    ...
    observations = {
        ...
        "account_state": torch.randn(batch_size, 16),
        "cost_state": torch.randn(batch_size, 1),
    }
